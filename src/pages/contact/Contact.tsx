import './Contact.css';

const Contact = () => {
  return (
    <main className="contact-page">
      <div className="contact-inner">
        <div className="a-text">
          Feel free to reach me through my personal email:
          <a href="mailto:chloecocoyip@gmail.com"> chloecocoyip@gmail.com</a>
          <br /><br />
          <strong>Links:</strong><br />
          - <a href="https://drive.google.com/drive/u/0/folders/1moOIsxQ4VQhvMhSNczqZmE3Zh8cGPA9O" target="_blank" rel="noopener noreferrer">Epic new Software Engineering Resume</a><br />
          - <a href="https://github.com/papaya830" target="_blank" rel="noopener noreferrer">GitHub</a><br />
          - <a href="https://www.linkedin.com/in/chloe-yip830/" target="_blank" rel="noopener noreferrer">LinkedIn</a><br />
        </div>
      </div>
    </main>
  );
};

export default Contact;